...
export default function HeroCarousel({
  ...
  return (
    <Card className="overflow-hidden p-0">
          ...
          {slides.map((slide, slideIndex) => (
            <div
              ...
              className={`absolute inset-0 transition-[opacity,transform] duration-700 ease-out ${
                slideIndex === index ? "opacity-100 translate-y-0 scale-100" : "pointer-events-none opacity-0 translate-y-3 scale-[1.02]"
              }`}
            >
              <Image
                src={slide.image.src}
        ...
        <div className="hidden grid gap-3 p-4 md:p-5 lg:grid">
          {slides.map((slide, slideIndex) => (
            <button
              ...
              className={`overflow-hidden rounded-3xl border text-left transition duration-300 ${
                slideIndex === index ? "border-[color:var(--neon)] shadow-[0_0_0_1px_rgba(245,224,29,.55)]" : "border-[color:var(--border)]"
              }`}
            >
              <div className="flex items-center gap-3 p-3">
                ...
                <div className="min-w-0">
                  {slide.eyebrow ? <p className="text-[10px] uppercase tracking-[0.28em] text-[color:var(--muted)]">{slide.eyebrow}</p> : null}
      ...
      <div className="flex items-center justify-center gap-2 border-t border-[color:var(--border)] bg-black/10 px-4 py-3">
        {slides.map((slide, slideIndex) => (
          <button
            key={slide.title}
            type="button"
            ...
            className={`h-2.5 rounded-full transition-all duration-300 ${
              slideIndex === index ? "w-10 bg-[color:var(--neon)]" : "w-2.5 bg-white/40"
            }`}
